 ...
 * rendering. Mirrors the minimum-viable subset of
 * `plans/DH-21476-06-shortest-path-customizable-table-options.md` and
 * Phase 1 of `plans/DH-21476-05-sidebar-plugin-extensibility.md`.
 *
 * Covers:
 ...
 * mounted in the DOM. Uses the controllable `isMenuShown` field
 * directly to avoid a click-driven path.
 */
function openMenu(grid: IrisGrid): void {
  act(() => {
    grid.applyState('isMenuShown', true, 'external');
  });
}

/**
 * Push a sidebar page onto the open-options stack via the controllable
 * `openOptions` field so the page-switch render path runs.
 */
function pushOpenOption(grid: IrisGrid, option: OptionItem): void {
  act(() => {
    grid.applyState(
      'openOptions',
      [option] as readonly OptionItem[],
      'external'
    );
  });
}
