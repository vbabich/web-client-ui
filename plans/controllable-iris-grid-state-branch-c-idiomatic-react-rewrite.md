...
> **Depends on**: [Phase 0](./controllable-iris-grid-state.md#phase-0--shared-foundation-both-branches-build-on-this) (registry only — the rest is replaced by this branch's store).
> **Spike scope (per [process plan, Step 3](./controllable-iris-grid-state-process.md#step-3--branch-c-feasibility-spike-parallel-separate-shape))**:
...
> **Companion branches**: [Branch A](./controllable-iris-grid-state-branch-a-imperative-ref.md), [Branch B](./controllable-iris-grid-state-branch-b-expanded-override.md).
> **Quick commands**:
>
> ```bash
> npm run types
...
plugins, so the [sidebar plugin plan](./controllable-iris-grid-table-options-plugin.md)
...
[controllable-iris-grid-table-options-plugin.md](./controllable-iris-grid-table-options-plugin.md)).
